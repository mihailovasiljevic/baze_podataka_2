...
CREATE OR REPLACE VIEW
adresa_nepokretnosti(Ulica, Broj, Grad, Drzava) AS
...
CREATE OR REPLACE VIEW
citava_nepokretnost(Oznaka,Povrsina, Useljiva, "U izgradnji", "Broj soba", "Broj spratova", Dupleks, 
          "Ima lift", "Stara gradnja", Legalna, Tajna, Prodata, Opis, "Tip nepokretnosti", "Oznaka prodavca",
          "Ime prodavca", "Prezime prodavca", "Broj licne karte prodavca", "Telefon prodavca", "Email prodavca", Ulica,Broj,Grad, Drzava, "Ukupni troskovi", "Ukupno slika"
) AS
SELECT DISTINCT N.NEK_ID, N.NEK_POV, N.NEK_USELJ, N.NEK_UIZGR, N.NEK_SOBE, N.NEK_SPRAT, N.NEK_DUPL, N.NEK_LIFT, N.NEK_STARA, N.NEK_LEG,
       N.NEK_TAJNA, N.NEK_PROD, N.NEK_OPIS, T.NEK_TIP_NAZ, P.PROD_ID, P.PROD_IME, P.PROD_PREZ, P.PROD_BRLK, P.PROD_TEL, P.PROD_EMAIL, U.ULICA_NAZ, A.ADRS_ULBR, G.LOK_NAZ, D.DRZ_NAZ, UOP.POSR_TROSK, nvl(count(SL.SLIKA_ID), '0')
...
U.ULICA_NAZ, A.ADRS_ULBR, G.LOK_NAZ, D.DRZ_NAZ, UOP.POSR_TROSK;
